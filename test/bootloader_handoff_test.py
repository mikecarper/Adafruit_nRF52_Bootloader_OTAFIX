...
if "flash_nrf5x_erase_step" in flash_source or "flash_nrf5x_erase_step" in ghostfat_source:
    raise AssertionError("UF2 must not use the hardware-failing partial-page erase path")
...
program_case = ghostfat_source.index("case UF2_APP_FLASH_PROGRAM_BLOCK:", erase_case)
erase_block = ghostfat_source[erase_case:program_case]
if "flash_nrf5x_erase(block->targetAddr, CODE_PAGE_SIZE);" not in erase_block:
    raise AssertionError("UF2 application update must erase one complete page per retry")
