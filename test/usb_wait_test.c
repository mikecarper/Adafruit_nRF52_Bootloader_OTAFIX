...
int main(void) {
  ...
  printf("[1] battery-only recovery does not wait: ");
  ...
  printf("[2] delayed host enumeration within 30 seconds succeeds: ");
  int before = failures;
  ...
  printf("[3] enumeration exactly at the deadline succeeds: ");
  before = failures;
  ...
  printf("[4] powered USB without a data host falls back after 30 seconds: ");
  before = failures;
  ...
  printf("[5] VBUS removal beats a stale mounted flag before the deadline: ");
  before = failures;
  ...
  printf("[6] VBUS removal beats a stale mounted flag at the deadline: ");
  before = failures;
